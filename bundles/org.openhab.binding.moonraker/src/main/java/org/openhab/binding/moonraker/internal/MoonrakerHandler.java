...
public class MoonrakerHandler extends BaseThingHandler implements EventListener {
    ...
    public void onError(final Throwable cause) {
        if (cause instanceof Exception) {
            ...
            logger.warn("Unexpected throwable: {0}", cause);
        }
    }
    ...
    private void updatePosition(BigDecimal[] values, int index, String group, String key, String postfix) {
        ...
        this.updateState(channel.getUID(), new QuantityType<>(values[index].scaleByPowerOfTen(-3), SIUnits.METRE));
    }
                            ...
                            switch (entry2.getKey()) {
                                ...
                                case "measured_max_temp":
                                    this.updateState(channel.getUID(),
                                            new QuantityType<>(
                                                    value.getAsBigDecimal().setScale(1, RoundingMode.HALF_UP),
                                                    SIUnits.CELSIUS));
                                    break;
                                case "rpm":
                                    this.updateState(channel.getUID(), new QuantityType<>(
                                ...
                                case "last_z_result":
                                    this.updateState(channel.getUID(), new QuantityType<>(
                                            value.getAsBigDecimal().scaleByPowerOfTen(-3), SIUnits.METRE));
                                    break;
                                case "filament_used":
                                    this.updateState(channel.getUID(),
                                            new QuantityType<>(value.getAsBigDecimal(), SIUnits.METRE));
                                    break;
                                case "file_position":
                ...
                if (value.isNumber()) {
                    switch (entry.getKey()) {
                        ...
                        case "first_layer_extr_temp":
                            this.updateState(channel.getUID(), new QuantityType<>(
                                    value.getAsBigDecimal().setScale(1, RoundingMode.HALF_UP), SIUnits.CELSIUS));
                            break;
                        case "estimated_time":
                            this.updateState(channel.getUID(), new QuantityType<>(
                                    value.getAsBigDecimal().setScale(0, RoundingMode.HALF_UP), Units.SECOND));
                        ...
                        case "filament_total":
                            this.updateState(channel.getUID(),
                                    new QuantityType<>(value.getAsBigDecimal().scaleByPowerOfTen(-3), SIUnits.METRE));
                            break;
                        case "size":
     ...
     * @return
     */
    private static @Nullable String substringBefore(@Nullable final String str, final int separator) {
     ...
     * @return the message
     */
    private static @Nullable String getRootCauseMessage(final @Nullable Throwable th) {
